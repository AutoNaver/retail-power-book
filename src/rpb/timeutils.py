...
def is_utc(index: pd.DatetimeIndex) -> bool:
    """True if `index` is timezone-aware with a UTC zone and a zero offset at every timestamp.

    This accepts any UTC alias ("UTC", "Etc/UTC", "GMT", `datetime.timezone.utc`)
    by checking offsets rather than the zone's name. The zone itself must have a
    zero offset in both January and July, so an empty index in a local zone such
    as Europe/Berlin or Europe/London is rejected too.
    """
    if index.tz is None:
        return False
    probes = pd.DatetimeIndex(["2000-01-01", "2000-07-01"]).tz_localize(index.tz)
    if any(ts.utcoffset() for ts in probes):
        return False
